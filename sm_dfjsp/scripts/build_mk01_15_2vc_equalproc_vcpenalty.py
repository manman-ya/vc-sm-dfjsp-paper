...
def build_payload(mk_path: Path) -> dict:
    ...
    candidate_srus_by_type = {"T1": ["U1", "U3"], "T2": ["U2", "U4"]}
    ...
    for job in jobs_payload:
        ...
        for sid in job["candidate_srus"]:
            ...
            if is_cross:
                ...
                rate = float(penalty["cross_chain_cost_rate"])
    ...
    return {
        "instance_name": f"{mk.name}_mvc_2vc_2type_4sru_equalproc_vcpenalty",
        ...
        "notes": {
            ...
            "cross_chain_penalty_rule": "2VC symmetric fixed cross-chain penalty; variable rate is always 0.",
...
def _assert_sanity(payload: dict) -> None:
    for job in payload["jobs"]:
        ...
        if len(comp["candidate_srus"]) != 2 or len(comp["intra_chain_srus"]) != 1 or len(comp["cross_chain_srus"]) != 1:
    ...
    for jkey, by_sru in payload["cross_chain"].items():
        for sid, info in by_sru.items():
            ...
            expected = 200.0 if info["is_cross_chain"] else 0.0
...
def _write_readme(output_dir: Path) -> None:
    text = """# MVC-MK01-15 2VC/2Type/4SRU Equal-Processing Dataset
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build MK01-15 as 2VC/2type/4SRU equal-processing MVC instances.")
    parser.add_argument("--mk-dir", default="data/mk")
    parser.add_argument("--output-dir", default="data/mvc_mk01_15_2vc4sru_equalproc_vcpenalty")
    parser.add_argument("--pattern", default="mk*.txt")
    args = parser.parse_args()

    mk_dir = _resolve(args.mk_dir)
    output_dir = _resolve(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    rows: list[dict] = []
    for mk_path in sorted(mk_dir.glob(args.pattern)):
        payload = build_payload(mk_path)
        _assert_sanity(payload)
        out_path = output_dir / f"{payload['source_instance']}_mvc_2vc_2type_4sru_equalproc_vcpenalty.json"
        ...
        rows.append(
            {
                ...
                "srus": payload["n_srus"],
                "ops": sum(int(j["n_operations"]) for j in payload["jobs"]),
                "seed": payload["seed"],
