...
from build_mk01_15_2vc_equalproc_vcpenalty import (
    SEED,
    _assign_types_by_operation_count,
    _job_lookup,
    _machine_costs,
    _pair_key,
    _resolve,
...
N_TYPES = 2
N_SRUS = 6
INSTANCE_SUFFIX = "mvc_3vc_2type_6sru_equalproc_vcpenalty"
...
def build_payload(mk_path: Path) -> dict:
    ...
    return {
        "instance_name": f"{mk.name}_{INSTANCE_SUFFIX}",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build mk14_mk15_merged as a 3VC/2type/6SRU equal-processing MVC instance.")
    parser.add_argument("--mk-file", default="data/mk/mk14_mk15_merged.txt")
    parser.add_argument("--output-dir", default="data/mvc_mk14_mk15_merged_3vc6sru_equalproc_vcpenalty")
    args = parser.parse_args()

    mk_path = _resolve(args.mk_file)
    output_dir = _resolve(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    payload = build_payload(mk_path)
    _assert_sanity(payload)
    out_path = output_dir / f"{payload['source_instance']}_{INSTANCE_SUFFIX}.json"
